...
class _EthereumState extends State<Ethereum> {
  ...
  double _exchangeRateETHtoNGN = 0.0;
  double _exchangeRateETHtoUSD = 0.0;
  String _selectedCurrency = 'ETH'; // Track the selected currency
  double _usdEquivalent = 0.0; // Store the USD equivalent of the entered ETH

  @override
  void initState() {
    super.initState();
    _fetchExchangeRates();
    _ethController.addListener(_convertCurrencyToNGN);
  }

  @override
  void dispose() {
    _ethController.removeListener(_convertCurrencyToNGN);
  ...
  Future<void> _fetchExchangeRates() async {
    try {
      ...
      if (responseNGN.statusCode == 200 && responseUSD.statusCode == 200) {
        ...
        if (mounted) {
          setState(() {
            _exchangeRateETHtoNGN =
                (dataNGN['ethereum']['ngn'] as num).toDouble();
            _exchangeRateETHtoUSD =
                (dataUSD['ethereum']['usd'] as num).toDouble();
            _updateExchangeRate();
          });
        }
      } else {
      ...
      if (mounted) {
        setState(() {
          ...
          _exchangeRateETHtoNGN = 0.0;
          _exchangeRateETHtoUSD = 0.0;
        });
      }
    }
  }
  ...
  void _convertCurrencyToNGN() {
    final amount = double.tryParse(_ethController.text) ?? 0.0;
    final ngnAmount = amount * _exchangeRateETHtoNGN;
    final usdAmount = amount * _exchangeRateETHtoUSD;
